...
_SU2NN_COMPILE_MODE = '__su2nn_compile_mode__'
...
def compile_mode(mode: str):
    ...
    def decorator(obj):
        if not (inspect.isclass(obj) and issubclass(obj, torch.nn.Module)):
            raise TypeError('@su2nn.util.jit.compile_mode can only decorate classes derived from torch.nn.Module')
        setattr(obj, _SU2NN_COMPILE_MODE, mode)
        return obj
...
def get_compile_mode(mod: torch.nn.Module) -> str:
    ...
    '''
    if hasattr(mod, _SU2NN_COMPILE_MODE):
        mode = getattr(mod, _SU2NN_COMPILE_MODE)
    else:
        mode = getattr(type(mod), _SU2NN_COMPILE_MODE, None)
...
def trace_module(mod: torch.nn.Module, inputs: dict = None, check_inputs: list = None, in_place: bool = True):
    ...
    old_mode = getattr(mod, _SU2NN_COMPILE_MODE, None)
    ...
    setattr(mod, _SU2NN_COMPILE_MODE, 'trace')
    ...
    if old_mode is not None:
        setattr(mod, _SU2NN_COMPILE_MODE, old_mode)
...
def script(mod: torch.nn.Module, in_place: bool = True):
    ...
    setattr(mod, _SU2NN_COMPILE_MODE, 'script')
