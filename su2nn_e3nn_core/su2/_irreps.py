...
class Irrep(tuple):
    r'''Irreducible representation of :math:`SU(2)`
    ...
    def __new__(cls, l: Union[int, "Irrep", str, tuple], p = None, t = None):
        if p is None or t is None:
            ...
            if isinstance(l, str):
                try:
                    ...
                    l = Fraction(name[:-2])
                    assert l >= 0
                    assert not (name[-2] == 'y' and l != int(l))
                    p = {
                        "e": 1,
                        "o": -1,
                        "y": (-1) ** l,
        ...
        if not (isinstance(l, int) or 2 * l == int(2 * l)) or l < 0:
            raise ValueError(f'l must be non-negative integer/half-integer, got {l}')
        if p not in (-1, 1):
            raise ValueError(f"parity must be on of (-1, 1), got {p}")
        if t not in (-1, 1):
            raise ValueError(f'time reversal must be one of (-1, 1), got {t}')
        return tuple.__new__(cls, (Fraction(l), p, t))
    ...
    @property
    def t(self) -> int:
        r'''The parity of the representation, :math:`t = \pm 1`.'''
        return self[2]
    ...
    def iterator(cls, lmax = None):
        r'''Iterator through all the irreps of :math:`SU(2) x P x T`'''
        for l in itertools.count(0, Fraction(1/2)):
            if l > lmax:
                break
            yield Irrep(l, 1, 1)
            yield Irrep(l, 1,-1)
    ...
    def dim(self) -> int:
        ...
        return int(2 * self.l + 1)

    def is_scalar(self) -> bool:
        """Equivalent to ``l == 0, p == 1, and t == 1``"""
        return self.l == 0 and self.p == 1 and self.t == 1

    def __mul__(self, other):
        r'''Generate the Irrep's from the product of two Irrep's.

        Returns
        -------
        generator of `su2nn.su2.Irrep`
        '''
        other = Irrep(other)
        p = self.p * other.p
        t = self.t * other.t
        ...
        lmax = self.l + other.l
        for dl in range(int(lmax - lmin) + 1):
            yield Irrep(lmin + dl, p, t)
