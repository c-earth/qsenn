...
from su2nn import su2
from su2nn.math import germinate_formulas, orthonormalize, reduce_permutation
from su2nn.util import explicit_default_types
from su2nn.util.codegen import CodeGenMixin
from su2nn.util.jit import compile_mode
...
def _wigner_nj(*irrepss, normalization = 'component', filter_ir_mid = None, dtype = None, device = None):
    irrepss = [su2.Irreps(irreps) for irreps in irrepss]
    if filter_ir_mid is not None:
        filter_ir_mid = [su2.Irrep(ir) for ir in filter_ir_mid]
        ...
        *irrepss_left, normalization = normalization, filter_ir_mid = filter_ir_mid, dtype = dtype, device = device
        ...
        for mul, ir in irreps_right:
            for ir_out in ir_left * ir:
                ...
                C = su2.wigner_3j(ir_out.l, ir_left.l, ir.l, dtype = dtype, device = device)
...
class ReducedTensorProducts(CodeGenMixin, torch.nn.Module):
    ...
    filter_ir_out : list of `su2nn.su2.Irrep`, optional
        Optional, list of allowed irrep in the output

    filter_ir_mid : list of `su2nn.su2.Irrep`, optional
        Optional, list of allowed irrep in the intermediary operations

    **kwargs : dict of `su2nn.su2.Irreps`
    ...
    Attributes
    ----------
    irreps_in : list of `su2nn.su2.Irreps`
    ...
    def __init__(self, formula, filter_ir_out = None, filter_ir_mid = None, eps = 1e-9, **irreps):
        ...
        if filter_ir_out is not None:
            try:
                filter_ir_out = [su2.Irrep(ir) for ir in filter_ir_out]
            except ValueError:
                raise ValueError(f'filter_ir_out (={filter_ir_out}) must be an iterable of su2nn.su2.Irrep')

        if filter_ir_mid is not None:
            try:
                filter_ir_mid = [su2.Irrep(ir) for ir in filter_ir_mid]
            except ValueError:
                raise ValueError(f'filter_ir_mid (={filter_ir_mid}) must be an iterable of su2nn.su2.Irrep')

        f0, formulas = germinate_formulas(formula)

        irreps = {i: su2.Irreps(irs) for i, irs in irreps.items()}
        ...
        for ir, path, base_su2 in _wigner_nj(*[irreps[i] for i in f0], filter_ir_mid = filter_ir_mid, dtype = torch.float64):
        ...
        for ir in Ps:
            ...
            base_su2 = torch.stack([R for _, R in Ps[ir]])

            R = base_su2.flatten(2)  # [multiplicity, ir, input basis] (u,j,omega)
            ...
            for x in X:
                C = torch.einsum("u,ui...->i...", x, base_su2)
        ...
        for i, op in enumerate(tps):
            tp = su2.TensorProduct(op[0], op[1], op[2], [(0, 0, 0, 'uuu', False)])
