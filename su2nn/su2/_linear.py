...
class Linear(CodeGenMixin, torch.nn.Module):
    r'''Linear operation equivariant to :math:`SU(2)`
    ...
    internal_weights : bool
        whether the `su2nn.su2.Linear` should store its own weights. Defaults to ``True`` unless ``shared_weights`` is
        explicitly set to ``False``, for consistancy with `su2nn.su2.TensorProduct`.

    shared_weights : bool
        whether the `su2nn.su2.Linear` should be weighted individually for each input in a batch. Defaults to ``False``.
    ...
    biases : list of bool, optional
        ...
        If ``biases = True`` it gives bias to all scalars (l = 0, p = 1 and t = 1).

    Attributes
    ----------
    weight_numel : int
        the size of the weights for this `su2nn.su2.Linear`
    '''
    ...
    def __init__(
        ...
        irreps_in = su2.Irreps(irreps_in)
