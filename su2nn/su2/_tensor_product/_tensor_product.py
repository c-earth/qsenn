...
class TensorProduct(CodeGenMixin, torch.nn.Module):
    ...
    def __init__(
        ...
        self.irreps_in1 = su2.Irreps(irreps_in1)
        self.irreps_in2 = su2.Irreps(irreps_in2)
        self.irreps_out = su2.Irreps(irreps_out)
        ...
        if in1_var is None:
            ...
            assert len(in1_var) == len(self.irreps_in1), 'length of ir1_var must be equal to len(irreps_in1)'

        if in2_var is None:
            in2_var = [1.0 for _ in range(len(self.irreps_in2))]
        else:
            in2_var = [float(var) for var in in2_var]
            assert len(in2_var) == len(self.irreps_in2), 'length of ir2_var must be equal to len(irreps_in2)'

        if out_var is None:
            out_var = [1.0 for _ in range(len(self.irreps_out))]
        else:
            out_var = [float(var) for var in out_var]
            assert len(out_var) == len(self.irreps_out), 'length of out_var must be equal to len(irreps_out)'
        ...
        opt_defaults = su2nn.get_optimization_defaults()
    ...
    def visualize(
        ...
        r'''Visualize the connectivity of this `su2nn.su2.TensorProduct`
