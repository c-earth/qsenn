...
from su2nn.nn import Extract, Activation
...
class _Sortcut(torch.nn.Module):
    def __init__(self, *irreps_outs):
        super().__init__()
        self.irreps_outs = tuple(su2.Irreps(irreps).simplify() for irreps in irreps_outs)
        irreps_in = sum(self.irreps_outs, su2.Irreps([]))
...
class Gate(torch.nn.Module):
    ...
    def __init__(self, irreps_scalars, act_scalars, irreps_gates, act_gates, irreps_gated):
        super().__init__()
        irreps_scalars = su2.Irreps(irreps_scalars)
        irreps_gates = su2.Irreps(irreps_gates)
        irreps_gated = su2.Irreps(irreps_gated)
        ...
        self.mul = su2.ElementwiseTensorProduct(irreps_gated, irreps_gates)
