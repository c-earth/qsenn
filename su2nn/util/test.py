...
from su2nn.util.jit import compile, get_tracing_inputs, get_compile_mode, _MAKE_TRACING_INPUTS
...
try:
    # If pytest is available, define an su2nn pytest plugin
...
def random_irreps(
    ...
    for _ in range(n):
        ...
        this_irreps = su2.Irreps(this_irreps)
...
def format_equivariance_error(errors: dict) -> str:
    ...
    '''
    return '\n'.join(
                     '(parity_k={:d}, time_k={:d}, did_translate={}) -> max error={:.3e} in argument {}'.format(
                     int(k[0]), int(k[1]), bool(k[2]), float(v.max()), int(v.argmax()))
...
def assert_equivariant(func, args_in = None, irreps_in = None, irreps_out = None, tolerance = None, **kwargs) -> dict:
    ...
    args_in : list or None
        ...
        ``su2.Irreps`` and ``'cartesian'``, random test inputs will be generated.
    irreps_in : object
    ...
    **kwargs : kwargs
    ...
    Returns
    -------
    The same as ``equivariance_error``: a dictionary mapping tuples ``(parity_k, time_k, did_translate)`` to errors
    '''
...
def equivariance_error(
    ...
    irreps_in : list of `su2nn.su2.Irreps` or `su2nn.su2.Irreps`
    ...
    irreps_out : list of `su2nn.su2.Irreps` or `su2nn.su2.Irreps`
    ...
    Returns
    -------
    dictionary mapping tuples ``(parity_k, time_k, did_translate)`` to an array of errors,
    ...
    tests = list(itertools.product(parity_ks, time_ks, do_translation))
    ...
    for trial in range(ntrials):
        for this_test in tests:
            parity_k, time_k, this_do_translate = this_test

            # Build a rotation matrix for point data
            rot_mat = su2.rand_matrix(dtype = transform_dtype)
